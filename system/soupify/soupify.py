...
def get_webpage(url, params=None, verify=False, timeout=10):
    ...
    try:
        requests_made+=1
        if post:
            page = requests.post(url, data=params, headers=headers, proxies=syt.PROXIES)
        else:
            page = requests.get(url, params=params, headers=headers, verify=verify, timeout=timeout, proxies=syt.PROXIES)
        syt.log_note(url)
    except:
        try:
            requests_made+=1
            if post:
                ...
                page = requests.get(url, params=params, headers=headers, verify=verify, timeout=timeout*2, proxies=syt.PROXIES)
            syt.log_note(url)
        except:
            invalid_urls += 1
