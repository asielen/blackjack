...
def make_project_path(string=""):
    ...
    """
    path = os.path.dirname(os.path.realpath(__file__)).split(project_directory)[0]
    if string != "":
        string = string.replace("/", os.sep)  # To fix mac > pc
        string = string.replace("\\", os.sep)  # To fix pc > mac
        return os.path.abspath(path+project_directory+os.sep+string)
    else:
        return os.path.abspath(path+project_directory+os.sep)
